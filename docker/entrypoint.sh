...
ONLINE_PROCESSORS_COUNT="$(getconf _NPROCESSORS_ONLN)"
...
cmake --build "BUILD/RELEASE-${SYSTEMC_CI_TARGET}/BUILD/" --parallel "${ONLINE_PROCESSORS_COUNT}"
cmake --install "BUILD/RELEASE-${SYSTEMC_CI_TARGET}/BUILD/"

if [[ "$BUILD_REGRESSIONS" == "true" ]]; then
  cmake --build "BUILD/RELEASE-${SYSTEMC_CI_TARGET}/BUILD/" --parallel "${ONLINE_PROCESSORS_COUNT}" --target check
fi
